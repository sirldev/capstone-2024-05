...
import { Container, Textarea, Text, Tooltip, Button, Group, AppShell, Burger, Grid, SimpleGrid, Paper } from '@mantine/core';
...
export default function UserInput({ onButtonClick } : UserInputProps ) {
  ...
  return (
        <Container  className={classes.inner}>
            ...
            <Text size="sm" c="dimmed" lh={1.6}>
            프로젝트에 적용되어야 하는 특정 제약사항이나 필요 조건이 있다면, 이를 분명히 명시해 주세요. 예를 들어, 특정 지역에서만 리소스를 생성해야 하거나, 비용 제한이 있는 경우 등입니다.
            </Text>
            ...
            {isDisabled ? (
                ...
                <div>
                    <Button disabled onClick={(event) => event.preventDefault()} mt={10}>
                        템플릿 생성하기
                    </Button>
                </div>
                </Tooltip.Floating>
