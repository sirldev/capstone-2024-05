...
인스턴스 요구 사항. 인스턴스 요구 사항을 지정하면 Amazon EC2는 제공된 요구 사항이 있는 인스턴스 유형을 식별한 다음 온디맨드 및 스팟 할당 전략을 사용하여 인스턴스 유형 목록을 지정할 때와 동일한 방식으로 이러한 인스턴스 유형에서 인스턴스를 시작합니다.  
...
`OnDemandAllocationStrategy` 로 설정된 `prioritized` 경우 스팟 집합은 우선 순위를 사용하여 온디맨드 용량을 이행할 때 가장 먼저 사용할 시작 템플릿 재정의를 결정합니다.  
스팟이 로 `AllocationStrategy` 설정된 `capacityOptimizedPrioritized` 경우 스팟 집합은 스팟 용량을 이행하는 데 사용할 시작 템플릿 재정의를 결정하기 위해 최선을 다해 우선 순위를 사용하지만 용량을 먼저 최적화합니다.  
