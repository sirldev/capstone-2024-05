# AWS::EC2::SpotFleet SpotFleetLaunch사양

하나 이상의 스팟 인스턴스에 대한 시작 사양을 지정합니다. 플릿 요청에 온디맨드 용량을 포함하면 사용할 수 없습니다 `SpotFleetLaunchSpecification`. LaunchTemplateConfig를 사용해야 합니다.
...
`BlockDeviceMappings`  
스팟 인스턴스에 매핑된 하나 이상의 블록 장치. 스냅샷 ID와 암호화 값을 모두 지정할 수는 없습니다. 이는 빈 볼륨만 생성 시 암호화할 수 있기 때문입니다. 스냅샷이 볼륨의 기반인 경우 스냅샷은 비어 있지 않으며 해당 암호화 상태가 볼륨 암호화 상태에 사용됩니다.  
*필수 항목 *여부: 아니요  
_형식_: BlockDeviceMapping 목록  
_업데이트 필요 사항_: 바꿔 놓음

`EbsOptimized`  
인스턴스가 EBS I/O에 최적화되어 있는지 여부를 나타냅니다. 이 최적화는 Amazon EBS에 전용 처리량과 최적화된 구성 스택을 제공하여 최적의 EBS I/O 성능을 제공합니다. 이 최적화는 모든 인스턴스 유형에서 사용할 수 있는 것은 아닙니다. EBS 최적화 인스턴스를 사용하는 경우 추가 사용 요금이 적용됩니다.  
기본값: `false`  
_필수_: 아니요  
_유형_: Boolean  
_업데이트 필요 사항_: 바꿔 놓음
...
IAM 인스턴스 프로파일입니다.  
*필수 항목 *여부: 아니요  
_형식_: IamInstanceProfileSpecification  
_업데이트 필요 사항_: 바꿔 놓음
...
인스턴스 유형에 대한 속성입니다. 인스턴스 속성을 지정하면 Amazon EC2가 해당 속성으로 인스턴스 유형을 식별합니다.  
를 지정`InstanceRequirements`하면 를 지정할 수 없습니다`InstanceType`.
*필수 항목 *여부: 아니요  
_유형_: InstanceRequirementsRequest  
_업데이트 필요 사항_: 바꿔 놓음
...
`Monitoring`  
인스턴스에 대한 모니터링을 활성화하거나 비활성화합니다.  
*필수 항목 *여부: 아니요  
_유형_: SpotFleetMonitoring  
_업데이트 필요 사항_: 바꿔 놓음

`NetworkInterfaces`  
하나 이상의 네트워크 인터페이스. 네트워크 인터페이스를 지정하는 경우 네트워크 인터페이스를 사용하여 서브넷 ID 및 보안 그룹 ID를 지정해야 합니다.  
 `SpotFleetLaunchSpecification` 현재 Elastic Fabric Adapter(EFA)를 지원하지 않습니다. EFA를 지정하려면 LaunchTemplateConfig를 사용해야 합니다.
*필수 항목 *여부: 아니요  
_유형_: InstanceNetworkInterfaceSpecification 목록  
_업데이트 필요 사항_: 바꿔 놓음
...
`RamdiskId`  
RAM 디스크의 ID입니다. 일부 커널은 시작 시 추가 드라이버가 필요합니다. RAM 디스크를 지정해야 하는지 여부에 대한 정보는 커널 요구 사항을 확인하십시오. 커널 요구 사항을 찾으려면 AWS 리소스 센터를 참조하고 커널 ID를 검색하십시오.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
_업데이트 필요 사항_: 바꿔 놓음
...
`SubnetId`  
인스턴스를 시작할 서브넷의 ID입니다. 여러 서브넷을 지정하려면 쉼표를 사용하여 구분합니다. 예를 들어 "subnet-1234abcdeexample1, subnet-0987cdef6example2"입니다.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
_업데이트 필요 사항_: 바꿔 놓음
...
`UserData`  
인스턴스가 시작할 때 사용하는 base64로 인코딩된 사용자 데이터입니다. 사용자 데이터는 16KB로 제한됩니다.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
_업데이트 필요 사항_: 바꿔 놓음

`WeightedCapacity`  
지정된 인스턴스 유형에서 제공하는 단위 수입니다. 이러한 단위는 인스턴스 또는 vCPU, 메모리 또는 I/O와 같은 성능 특성과 관련하여 목표 용량을 설정하기 위해 선택한 것과 동일한 단위입니다.  
목표 용량을 이 값으로 나눈 값이 정수가 아닌 경우 Amazon EC2는 인스턴스 수를 다음 정수로 반올림합니다. 이 값을 지정하지 않으면 기본값은 1입니다.  
*필수 항목 *여부: 아니요  
_유형_: 더블  
_업데이트 필요 사항_: 바꿔 놓음
