...
EC2 집합의 시작 템플릿에 대한 재정의를 지정합니다.

`FleetLaunchTemplateOverridesRequest` 은 FleetLaunchTemplateConfigRequest 속성 유형의 속성입니다.
...
`InstanceType`  
인스턴스 유형입니다.  
 `mac1.metal` 는 시작 템플릿 재정의로 지원되지 않습니다.  
...
`Placement`  
해당하는 경우 인스턴스가 시작된 위치입니다.  
*필수 항목 *여부: 아니요  
_유형_: 놓기  
_업데이트 필요 사항_: 바꿔 놓음

`Priority`  
시작 템플릿 재정의의 우선 순위입니다. 우선 순위가 가장 높은 것이 먼저 시작됩니다.  
온디맨드가 `AllocationStrategy` 로 설정된 `prioritized` 경우 EC2 집합은 우선 순위를 사용하여 온디맨드 용량을 이행할 때 가장 먼저 사용할 시작 템플릿 재정의를 결정합니다.  
스팟이 `AllocationStrategy` 로 설정된 `capacity-optimized-prioritized` 경우 EC2 집합은 스팟 용량을 이행하는 데 사용할 시작 템플릿 재정의를 결정하기 위해 최선을 다해 우선 순위를 사용하지만 용량을 먼저 최적화합니다.  
유효한 값은 에서 시작하는 정수입니다`0`. 숫자가 낮을수록 우선 순위가 높습니다. 숫자를 설정하지 않으면 시작 템플릿 재정의의 우선 순위가 가장 낮습니다. 다른 시작 템플릿 재정의에 대해 동일한 우선 순위를 설정할 수 있습니다.  
*필수 항목 *여부: 아니요  
_유형_: 더블  
_업데이트 필요 사항_: 바꿔 놓음

`SubnetId`  
인스턴스를 시작할 서브넷의 ID입니다. 쉼표를 사용하여 여러 서브넷 ID를 구분합니다(예: `subnet-1234abcdeexample1, subnet-0987cdef6example2`). 유형의 요청에는 `instant` 서브넷 ID가 하나만 있을 수 있습니다.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
_업데이트 필요 사항_: 바꿔 놓음

`WeightedCapacity`  
지정된 인스턴스 유형에서 제공하는 단위 수입니다.  
*필수 항목 *여부: 아니요  
_유형_: 더블  
_업데이트 필요 사항_: 바꿔 놓음
...
- Amazon EC2 API 참조*의 FleetLaunchTemplateOverridesRequest *
