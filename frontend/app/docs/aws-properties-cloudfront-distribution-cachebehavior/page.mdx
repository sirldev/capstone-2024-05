# AWS::CloudFront::D istribution CacheBehavior

CloudFront에서 요청을 처리하는 방법을 설명하는 복합 유형입니다.

CloudFront에서 모든 오리진의 객체를 제공하려면 최소한 오리진의 캐시 동작(기본 캐시 동작 포함)을 생성해야 합니다. 각 캐시 동작은 CloudFront에서 객체를 가져올 하나의 오리진을 지정합니다. 오리진이 두 개이고 기본 캐시 동작만 있는 경우, 기본 캐시 동작으로 인해 CloudFront는 오리진 중 하나에서 객체를 가져오지만 다른 오리진은 사용되지 않습니다.

배포에 추가할 수 있는 캐시 동작 수에 대한 현재 할당량(이전에는 제한이라고 함) 은 _Amazon CloudFront 개발자 안내서의 할당량을 참조하십시오_.

캐시 동작을 지정하지 않으려면 빈 요소만 포함합니다 `CacheBehaviors` . 빈 `CacheBehavior` 요소는 유효하지 않으므로 포함하지 마세요.

기존 배포의 모든 캐시 동작을 삭제하려면 배포 구성을 업데이트하고 빈 요소만 포함합니다 `CacheBehaviors` .

하나 이상의 캐시 동작을 추가, 변경 또는 제거하려면 배포 구성을 업데이트하고 업데이트된 배포에 포함할 모든 캐시 동작을 지정합니다.

캐시 동작에 대한 자세한 내용은 _Amazon CloudFront 개발자 안내서의 캐시 동작 설정을 참조하십시오_.
...
`CachePolicyId`  
이 캐시 동작에 연결된 캐시 정책의 고유 식별자입니다. 자세한 내용은 Amazon CloudFront 개발자 안내서의 캐시 정책 생성 또는 관리 캐시 정책 사용을 참조하세요\**.  
A 는 `CacheBehavior` a 또는 를 `CachePolicyId` 포함해야 합니다`ForwardedValues`. 를 사용하는 것이 좋습니다`CachePolicyId`.  
*필수*: 조건부  
*유형*: 문자열  
*업데이트 필요 사항\*: 중단 없음

`Compress`  
CloudFront에서 이 캐시 동작에 대해 특정 파일을 자동으로 압축할지 여부입니다. 그렇다면 true를 지정합니다. 그렇지 않은 경우 false를 지정합니다. 자세한 내용은 _Amazon CloudFront 개발자 안내서의 압축 파일 제공을 참조하십시오_.  
*필수 항목 *여부: 아니요  
_유형_: Boolean  
_업데이트 필요 사항_: 중단 없음
...
CloudFront `ID` 에서 이 캐시 동작에 대한 특정 데이터 필드를 암호화하는 데 사용할 필드 수준 암호화 구성의 값입니다.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
_업데이트 필요 사항_: 중단 없음
...
`MinTTL`  
이 필드는 더 이상 사용되지 않습니다. `MinTTL` 캐시 정책에서 이 필드 대신 이 필드를 사용하는 것이 좋습니다. 자세한 내용은 Amazon CloudFront 개발자 안내서의 캐시 정책 생성 또는 관리 캐시 정책 사용을 참조하세요\*_.  
객체가 업데이트되었는지 여부를 확인하기 위해 CloudFront에서 오리진에 다른 요청을 전달하기 전에 객체를 CloudFront 캐시에 유지하려는 최소 시간입니다. 자세한 내용은 _ Amazon CloudFront 개발자 안내서*의 콘텐츠가 엣지 캐시에 유지되는 기간(만료) 관리를 참조하십시오.  
...
이 캐시 동작에 연결된 오리진 요청 정책의 고유 식별자입니다. 자세한 내용은 Amazon CloudFront 개발자 안내서의 오리진 요청 정책 생성 또는 관리형 오리진 요청 정책 사용을 참조하세요\**.  
*필수 항목 *여부: 아니요  
*유형*: 문자열  
*업데이트 필요 사항\*: 중단 없음

`PathPattern`  
`images/*.jpg`동작을 적용할 요청을 지정하는 패턴(예: )입니다. CloudFront에서 최종 사용자 요청을 수신하면 요청된 경로가 배포에 캐시 동작이 나열된 순서대로 경로 패턴과 비교됩니다.  
선택적으로 `/`경로 패턴의 시작 부분에 슬래시()를 포함할 수 있습니다. 예를 들어, `/images/*.jpg`. CloudFront 동작은 선행 `/`.
기본 캐시 동작에 대한 경로 패턴은 `*` 변경할 수 없습니다. 객체에 대한 요청이 캐시 동작의 경로 패턴과 일치하지 않는 경우, CloudFront에서는 기본 캐시 동작의 동작을 적용합니다.  
자세한 내용은 * Amazon CloudFront 개발자 안내서*의 경로 패턴을 참조하십시오.  
_필수 항목 여부_: 예  
_유형_: 문자열  
_업데이트 필요 사항_: 중단 없음
...
`TargetOriginId`  
 `ID` CloudFront에서 요청이 이 캐시 동작과 일치할 때 요청을 라우팅할 오리진의 값입니다.  
_필수 항목 여부_: 예  
_유형_: 문자열  
_업데이트 필요 사항_: 중단 없음
...
- Amazon CloudFront API Reference\* \*의 CacheBehavior
