# AWS::EC2::CapacityReservationFleet 인스턴스 유형 사양

용량 예약 집합에서 사용할 인스턴스 유형에 대한 정보를 지정합니다.

`InstanceTypeSpecification` 는 AWS::EC2::CapacityReservationFleet 리소스의 속성입니다.
...
`AvailabilityZone`  
용량 예약 플릿이 용량을 예약하는 가용 영역입니다. 용량 예약 플릿은 가용 영역에 걸쳐 있을 수 없습니다. 플릿에 대해 지정하는 모든 인스턴스 유형 사양은 동일한 가용 영역을 사용해야 합니다.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
_업데이트 필요 사항_: 바꿔 놓음

`AvailabilityZoneId`  
용량 예약 플릿이 용량을 예약하는 가용 영역의 ID입니다. 용량 예약 플릿은 가용 영역에 걸쳐 있을 수 없습니다. 플릿에 대해 지정하는 모든 인스턴스 유형 사양은 동일한 가용 영역을 사용해야 합니다.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
_업데이트 필요 사항_: 바꿔 놓음

`EbsOptimized`  
용량 예약 집합이 EBS 최적화 인스턴스 유형을 지원하는지 여부를 나타냅니다. 이 최적화는 Amazon EBS에 전용 처리량과 최적화된 구성 스택을 제공하여 최적의 I/O 성능을 제공합니다. 이 최적화는 모든 인스턴스 유형에서 사용할 수 있는 것은 아닙니다. EBS 최적화 인스턴스 유형을 사용할 때 추가 사용 요금이 적용됩니다.  
*필수 항목 *여부: 아니요  
_유형_: Boolean  
_업데이트 필요 사항_: 바꿔 놓음

`InstancePlatform`  
용량 예약 집합이 용량을 예약하는 운영 체제의 유형입니다.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
...
`Priority`  
인스턴스 유형에 할당할 우선 순위입니다. 이 값은 플릿에 지정된 인스턴스 유형 중 어떤 인스턴스 유형을 우선적으로 사용해야 하는지 결정하는 데 사용됩니다. 값이 낮을수록 우선 순위가 높습니다. 자세한 내용은 Amazon EC2 사용 설명서의 인스턴스 유형 우선 순위를 참조하세요.  
*필수 항목 *여부: 아니요  
_유형_: Integer  
_최소_: `0`  
_업데이트 필요 사항_: 바꿔 놓음
...
`Weight`  
지정된 인스턴스 유형에서 제공하는 용량 단위의 수입니다. 이 값은 플릿에 대해 지정하는 총 목표 용량과 함께 플릿이 용량을 예약하는 인스턴스 수를 결정합니다. 두 값 모두 워크로드에 적합한 단위를 기반으로 합니다. 자세한 내용은 Amazon EC2 사용 설명서의 총 목표 용량을 참조하세요.  
유효 범위: 의 최소값`0.001`입니다. 최대값. `99.999`  
*필수 항목 *여부: 아니요  
_유형_: 더블  
_업데이트 필요 사항_: 바꿔 놓음
