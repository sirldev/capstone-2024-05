# AWS::EC2::LaunchTemplate LaunchTemplateTag사양

만드는 동안 시작 템플릿에 적용할 태그를 지정합니다.

`LaunchTemplateTagSpecification` 은 AWS::EC2::LaunchTemplate의 속성입니다.
...
`ResourceType`  
리소스의 유형입니다. 시작 템플릿에 태그를 지정하려면 `ResourceType` 이어야 합니다`launch-template`.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
