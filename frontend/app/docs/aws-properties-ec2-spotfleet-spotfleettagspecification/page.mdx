# AWS::EC2::SpotFleet SpotFleetTag사양
...
`ResourceType`  
리소스의 유형입니다. 현재 지원되는 유일한 리소스 종류는 `instance`입니다. 생성 시 스팟 집합 요청에 태그를 지정하려면 `TagSpecifications` 의 파라미터 를 사용합니다`SpotFleetRequestConfigData`.  
*필수 항목 *여부: 아니요  
_유형_: 문자열  
